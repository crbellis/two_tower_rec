...
movies = movies.batch(128)
print(movies.take(1).as_numpy_iterator().next())
# Define your objectives.
task = tfrs.tasks.Retrieval(
    metrics=tfrs.metrics.FactorizedTopK(movies.map(movie_model), name="metric")
)
...
index.index_from_dataset(
    movies.map(lambda title: (title, model.movie_model(title)))
)
